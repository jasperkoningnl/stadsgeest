...
*Cowork-update: 2026-06-04 — Historische backfill scripts aangemaakt: `scraper/src/run-backfill.js` (6 bronnen: Rechtspraak 12mnd, Officiële Bekendmakingen 6mnd, TenderNed 12mnd, Subsidieregister 3jr, CBS StatLine wijken, Jaarverslagen 10 org) + `scraper/src/run-backfill-browser.js` (Notubiz/Raadsinformatie 12mnd via Playwright). lib.js uitgebreid met is_historical parameter. Verwacht volume: ~1000-2500 items. Uitvoeren: `cd scraper && node src/run-backfill.js all` en los `node src/run-backfill-browser.js`. Intake daarna aanpassen: historische items → status 'watching' i.p.v. 'new'.*
...
*Code-update: 2026-06-03 — updates[] feature geïmplementeerd (PR #35): nieuw ArticleUpdates client-component toont updatebalk op artikelpagina (niet-inklapbaar bij één update, inklapbare geschiedenis bij meerdere); updates[] en updatedAt toegevoegd aan GROQ-queries; "bijgewerkt" label