...
async function main() {
  ...
  const sources = (await db.execute("SELECT id, name, url, expected_yield, health FROM sources WHERE COALESCE(health,'ok') != 'uitgeschakeld'")).rows;
  const regels = [];
  let nVerdacht = 0, nDood = 0;
  ...
  for (const s of sources) {
    ...
    if (leeg12) { health = 'dood'; note = `0 items of fout in laatste ${runs.length} runs`; }
    else if (leeg6 && yieldVerwacht > 0.3) { health = 'verdacht'; note = `0 items in laatste 6 runs, historisch levert deze bron in ${Math.round(yieldVerwacht * 100)}% van de runs`; }
    else if (fout6 >= 3) { health = 'verdacht'; note = `${fout6} van laatste 6 runs faalden`; }
  ...
  const kop = `# Bronnenwacht ${datum}${jobName ? ` (na job ${jobName})` : ''}\n\nGemeten in runs (niet kalendertijd). ${nVerdacht} verdacht, ${nDood} dood.\n\n`;
  fs.writeFileSync(path.join(dir, `rapport-${datum}.md`), kop + (regels.length ? regels.join('\n') : 'Geen afwijkingen.') + '\n', 'utf8');
  console.log(`Bronnenwacht: ${sources.length} bronnen gecheckt, ${nVerdacht} verdacht, ${nDood} dood. Rapport: bronnenwacht/rapport-${datum}.md`);
